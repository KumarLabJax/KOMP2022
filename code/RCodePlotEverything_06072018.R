...
######################### CONTROL OVER DATE #################################################################
	...
	pdf(paste("Control_",DataName,"_byDate.pdf", sep = ""), 14,4)			
			for (i in PhenoList) {
				...
				plot1 <- ggplot(aes(x = x,y = y), data = control) +
				geom_point(aes(colour = Sex), data = control, shape = 16,size = 1.0) +
				geom_line(size = 1.0, fun.data = mean_sdl,stat = 'summary') +
				scale_colour_brewer(guide = guide_legend(), palette = 'Set1') + labs(title = paste("ControlOnly_",i, sep = "")) +
				...
				plot2 <- ggplot(aes(x = x,y = y,colour = z),data=control) +
				geom_point(shape = 16,size = 1.0) +
				geom_line(size = 1.0, fun.data = mean_sdl,stat = 'summary') +
				#geom_line(aes(x = x,y = y, colour = z),data=control,size = 1.0,fun.data = mean_sdl,stat = 'summary') +
				scale_colour_brewer(guide = guide_legend(),palette = 'Set1',  name = j) + labs(title = paste("ControlOnly_",i, sep = "")) +
				#theme(axis.text.x = element_text(vjust = 0.5,hjust = 1.0,angle = 90.0)) + 
				theme_bw() + #scale_x_datetime(breaks = "3 month") +
				geom_hline(yintercept = controlMean, color = "grey", size = 1) + 
				geom_hline(yintercept = controlMean + controlSD, color = "grey", size = 1) + 
...
###########################  ALL OVER DATE  ###############################################################
	...
	pdf(paste("ALL_",DataName,"_byDate.pdf", sep = ""), 14,4)			
		for (i in PhenoList) {
				...
				plot1 <- ggplot(aes(x = x,y = y,colour = Sex), data=DataSet) +
				geom_point(shape = 16,size = 1.0) +
				geom_smooth(method = 'loess', span = 0.05, size = 1.0) +
				#geom_line(aes(x = x,y = y),data=DataSet,size = 1.0,fun.data = mean_sdl,stat = 'summary') +
				scale_colour_brewer(guide = guide_legend(), palette = 'Set1') + labs(title = paste("ALL_",i, sep = "")) +
				#theme(axis.text.x = element_text(vjust = 0.5,hjust = 1.0,angle = 90.0)) + 
				theme_bw() + #scale_x_datetime(breaks = "3 month") +
				geom_hline(yintercept = controlMean, color = "grey", size = 1) + 
				geom_hline(yintercept = controlMean + controlSD, color = "grey", size = 1) + 
			...
			for (j in ExperimentalFactors) {
				...
				plot2 <- ggplot(aes(x = x,y = y, colour = z),data=DataSet) +
				geom_point(shape = 16,size = 1.0) +
				#geom_smooth(method = 'loess', span = 0.05, size = 1.0) +
				geom_line(aes(x = x,y = y),data=DataSet,size = 1.0,fun.data = mean_sdl,stat = 'summary') +
				scale_colour_brewer(guide = guide_legend(),palette = 'Set1',  name = j) + labs(title = paste("ALL_",i, sep = ""))  +
				#theme(axis.text.x = element_text(vjust = 0.5,hjust = 1.0,angle = 90.0)) + 
				theme_bw() + #scale_x_datetime(breaks = "3 month") +
				geom_hline(yintercept = controlMean, color = "grey", size = 1) + 
				geom_hline(yintercept = controlMean + controlSD, color = "grey", size = 1) + 
...
###########################  ALL OVER TIME OF DAY ###############################################################
		pdf(paste("ALL_",DataName,"_byTimeOfDay.pdf", sep = ""), 12,4)			
			for (i in PhenoList) {
				...
				scale_colour_brewer(guide = guide_legend(),palette = 'Set1',  name = i) + labs(title = paste("ALL_",i, sep = "")) +
				theme(axis.text.x = element_text(vjust = 0.5,hjust = 1.0,angle = 90.0)) + 
				theme_bw() +
				geom_hline(yintercept = controlMean, color = "grey", size = 1) + geom_hline(yintercept = controlMean + controlSD, color = "grey", size = 1) + geom_hline(yintercept = controlMean - controlSD, color = "grey", size = 1)
...
#################### CONTROL STATS ######################################################################
		...
		### TryCatch will print the errors and go one with the function.  This is really useful.  
...
setwd("PhenotypePlots")		
		...
		for (i in PhenoList) {
			...
			pdf(paste("Pheno_",i,".pdf", sep = ""), 5,30 )			
				...
				# plot1 <- ggplot() +
				# geom_pointrange(aes(x = x,y = y, na.rm = TRUE),data= DataSet,colour = '#0000ff',fill = '#ffffff',size = 1.0, shape = 20, fun.data = mean_sdl,fun.args = list(mult = 1),stat = 'summary',na.rm=T) + 
				# theme_bw(base_size = 4.0) + labs(title = i) +
				# geom_hline(yintercept = controlMean, color  = "red") + 
				# geom_hline(yintercept = controlMean + controlSD, color  = "red") +
				# geom_hline(yintercept = controlMean - controlSD, color  = "red") +
...
####################plot all phenotypes one pdf	######################################################################
			...
			pdf(paste("ALL_Pheno_",DataName,".pdf", sep = ""), 5,30)			
...
####################plot all phenotypes one pdf	MARK KREBS ######################################################################

			pdf(paste("ALL_Pheno_Mark_",DataName,".pdf", sep = ""), 8,4)			
			...
			for (i in PhenoList) {
				...
				plot1 <- ggplot() +
				geom_linerange(aes(x = reorder(x, y, FUN = mean,na.rm=TRUE),y = y, na.rm = TRUE),data=subset(DataSet, !is.na(y)),colour = 'gray70',size = 0.2,fun.data = mean_sdl,fun.args = list(mult = 1),stat = 'summary') + 
				geom_point(aes(x = reorder(x, y, FUN = mean,na.rm=TRUE),y = y, na.rm = TRUE),data=subset(DataSet, !is.na(y)),colour = '#377eb8',size = 0.8,shape = 16,fun.data = mean_sdl,fun.args = list(mult = 1),stat = 'summary') +
				theme_bw(base_size = 10.0) + 
				#theme_void() +
				labs(title = i, x ="KOMP2 Lines", y= i) + #label X and Y axis
				theme(axis.text.x = element_blank())+ #remove X labels for each KOMP line name
				theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank()) + #remove grid lines
				geom_hline(yintercept = controlMean, color  = "#e41a1c") + 
				geom_hline(yintercept = controlMean + controlSD, color  = "#e41a1c", linetype = "dashed") +
				geom_hline(yintercept = controlMean - controlSD, color  = "#e41a1c", linetype = "dashed") 
				print(plot1)
...
# #################### Write Summary freq Table	######################################################################
# #### Colname StrainGeno starts with domain name, should use colname x :comment by Hao He 06/07/2018
					setwd(file.path(workDir, outDir, out1Dir))
					summary1 <- xtabs(~ x + Sex, data = DataSet)
					write.csv(summary1, file = "SummaryFreq.csv")
					rm(summary1)
